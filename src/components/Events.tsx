...
const Events = () => (
  ...
  <Link to="/request-quote" className="inline-block px-8 py-4 bg-primary text-background font-bold rounded-full shadow hover:bg-sage transition" onClick={scrollToTop}>Reserve Your Spot</Link>
    </header>
      ...
      <div className="grid md:grid-cols-3 gap-10">
        {events.map((event, idx) => (
          <div key={idx} className="bg-card rounded-3xl shadow-xl p-8 flex flex-col items-center text-center">
            ...
            <Link to="/request-quote" className="mt-auto px-6 py-2 bg-gradient-to-r from-golden to-sage text-background rounded-full font-semibold shadow hover:scale-105 transition" onClick={scrollToTop}>Book This Event</Link>
          </div>
        ))}
      </div>
    </section>
